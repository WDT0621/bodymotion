...
int main(void)
{
	...
	int i=0;
		...
		//	USART3_Init(9600);   	//串口3初始化为9600  激光扫平仪模块
			...
			STMFLASH_Read(FLASH_SAVE_ADDR,(u16*)datatemp,22);   //第0~7字节是ID,8~14字节是传感器网络地址
			...
			if(datatemp[i+8]==0xFF){  
					...
					STMFLASH_Write(FLASH_SAVE_ADDR,(u16*)datatemp,22);   //将绑定的网络地址初始化为0
			}else{
					for(i=0;i<14;i++) bindinf[i]=datatemp[i+8];				
			}																				
			for(i=0;i<14;i++){
					SensorNum = i;          //绑定的传感器数量
					if(bindinf[i]==0x00)
							break;
			}
