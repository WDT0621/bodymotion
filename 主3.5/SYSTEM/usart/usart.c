...
u8 frameLen1;           					//串口1帧长
u8 send_to_PC[9] = {0x67,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
static u8 SaveBuffer[22];//用于Flash存取
...
//输入：inOpCode：操作码
void BTSendCfg(u8 inOpCode)       //配置命令为02 C0 Code
{
		commSendBuff2[0]=2;
...
{
		commSendBuff2[0]=4;
	  commSendBuff2[1]=CONTROL_MSG_CONFIG; //C0 配置命令
...
{
		commSendBuff2[0]=4;
	  commSendBuff2[1]=CONTROL_MSG_TRANS;  //C2 透传命令
	  commSendBuff2[2]=InNodeAddr/256;
	  commSendBuff2[3]=InNodeAddr%256;
...
void FrameParse1(void){
    ...
    if(commStatus1 == COMM_FRAME_READY){	//通信状态————帧已接收，待处理
				cmdCode = commRcvBuff1[0];        //读取命令码
				...
				if(!CheckSumTest(frameLen1,commRcvBuff1))//校验和判断
				{
						...
						case COMM_SENSOR_STATUS:				//查询绑定状态-0x63--to test in Hex：63 00 63
									STMFLASH_Read(FLASH_SAVE_ADDR,(u16*)SaveBuffer,22);          //0~7位是ID,8~14位是绑定地址
									for(j=0;j<14;j++) bindinf[j] = SaveBuffer[j+8];																		
									commSendBuff1[0]=0X63;
									for(j=0;j<14;j++){
						          commSendBuff1[j+2]=bindinf[j];
									  SensorNum = j;          //绑定的传感器数量
									  if(bindinf[j]==0x00)
											 break;
									}
									commSendBuff1[1]=SensorNum;
									commSendBuff1[SensorNum+2] = GetCheckSum(SensorNum+3,commSendBuff1);//计算校验和
									SendToPC(SensorNum+3,commSendBuff1);				//发送绑定状态值
						break;				
						case COMM_BIND_BEGIN:		//绑定传感器   -0x64--to test in Hex：64 01 65  (绑定位置1的传感器)
									if(commRcvBuff1[1]==0||commRcvBuff1[1]>15){		//给的传感器位置判断
						...
						break;
						case COMM_BIND_ADDR:   //设置绑定地址 -0x68
						      if(commRcvBuff1[1]>14){		
											SendShortCmd(cmdCodeErr,COMM_POSITION_ERR);				//给的传感器位置错误
									}
									else{
										if(commRcvBuff1[1]==0){
										  ...
										  for(j=0;j<commRcvBuff1[1];j++)
									    {
										    bindinf[j]=commRcvBuff1[j+2];        //2~8字节为设备ID，存入suitMAC
										    SaveBuffer[j+8] = bindinf[j];//将需要的数据存入缓存变量
									    }
										}
									    STMFLASH_Write(FLASH_SAVE_ADDR,(u16*)SaveBuffer,22);
									    STMFLASH_Read(FLASH_SAVE_ADDR,(u16*)SaveBuffer,22);          //再读出进行检验
									    for(j=0;j<14;j++)                                  
									    {
											   if(bindinf[j] != SaveBuffer[j+8])break;
									    }
									    if (j<14){
											   SendShortCmd(cmdCodeErr,COMM_FLASH_ERR);
									    }else{
						...
						break;
						case COMM_BT_RESTORE:			//0x69		蓝牙模块恢复出厂配置									
								 BTSendCfg(FACTORY_RESTORE);
						break;									
						case COMM_BT_TRANSPORT:		//0x6A 上位机端使用的蓝牙透传指令					
								 btMode = 1;
								 BTSendData(commRcvBuff1[1]+1,commRcvBuff1+1);//第2字节+1为帧长，从第2字节开始发送
						break;
						case COMM_READ_ANGLE:		//读传感器的角度数据-0x67--to test in Hex：67 01 68  (读取位置1的传感器)
							  //QQQ超时需要加吗？
								if(commRcvBuff1[1]==0||commRcvBuff1[1]>0x0F)
								{
										SendShortCmd(cmdCodeErr,COMM_POSITION_ERR);										
								}else{
				...
				if(btMode==1){	//收到的蓝牙命令打包后发送到上位机
					  ...
					  for(i=0;i<tFrameLen;i++){
						...
						switch(msgCtlType){					
							...
							break;
							case RESPONSE_MSG_TRANS:	//透传消息 -0x42--目前只处理读角度消息
									if(commRcvBuff2[0]==13 && commRcvBuff2[6]==COMM_READ_ANGLE){										  	
										//send_to_PC[1] = commRcvBuff2[2];
										for(i=0;i<SensorNum;i++){
											if(commRcvBuff2[2] == datatemp[i+8]){
												send_to_PC[1] = i+1;
												break;
											}
										}
										for(i=0;i<6;i++){
											send_to_PC[i+2] = commRcvBuff2[i+7];
										}
										send_to_PC[8] = GetCheckSum(9,send_to_PC);
										SendToPC(9,send_to_PC);	//回传角度数据给上位机：帧格式在传感器端已打包
									}
							break;
	...
	OSIntEnter();    
		...
		{			
					  ...
					  commStatus1 = COMM_RECEIVING;
						switch(commRcvBuff1[0]){
							case COMM_SHAKEHAND:case COMM_READ_ID:case COMM_SENSOR_STATUS: case  COMM_BT_RESTORE:    //帧长为3字节
								...
								case COMM_BIND_ADDR:					//zhh bug 未加透传指令码的处理
										 frameLen1 = 15;	break;
				...
				if(commStatus1 == COMM_RECEIVING){
						if(((commRcvBuff1[0]==COMM_BT_TRANSPORT)|(commRcvBuff1[0]==COMM_BIND_ADDR))&& ptrRcv1==1){//在透传指令下，第二字节为帧长
